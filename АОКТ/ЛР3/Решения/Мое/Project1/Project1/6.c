...
void outputMemoryBaseData(long regdata)
{
  ...
  printf("Задают начальный и конечный адрес памяти, на кототором отражены регистры ввода-вывода устройств за мостом");
...
long readRegister(int bus, int device, int function, int reg) {
   // C помощью сдвигов формируем адрес нужного регистра
